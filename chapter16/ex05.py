...
def mul_time(time: Time, n: float) -> Time:
    """Принимает объект Time и число типа float и возвращает новый объект Time, который содержит произведение исходного времени и числа n.
    >>> time1 = Time()
...
def average_pace(time: Time, s: float) -> Time:
    """Принимает объект Time, представляющий время окончания гонки, и число типа float, представляющее расстояние,
